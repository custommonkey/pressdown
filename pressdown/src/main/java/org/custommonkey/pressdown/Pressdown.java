...
public class Pressdown {
    ...
    public static void main(final String[] args) throws Exception {
        ...
        try {
            ...
            if (args.length == 0) {
                new Pressdown().transform(file("README.md"));
            } else {

                final Pressdown app = new Pressdown(file(args[0]));

                for (int i = 1; i < args.length; i++) {
                    app.transform(file(args[i]));
                }
            }
    ...
    private static String file(String file) throws FileNotFoundException {
    ...
    Pressdown(final String xsl) throws TransformerConfigurationException,
            IOException {
    ...
    private static SAXSource markdownSource(final String args) {
        return new SAXSource(new SAXEventVisitor(), new InputSource(args));
    }
